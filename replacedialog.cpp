...
void ReplaceDialog::on_btFindNext_clicked()
{
    ...
    if(ui->rbDown->isChecked()){
        index=text.indexOf(target,c.position(),ui->cbCapital->isChecked() ? Qt::CaseSensitive : Qt::CaseInsensitive);
        if(index>=0){
            c.setPosition(index);
        ...
        index=text.lastIndexOf(target,c.position()-text.length()-1,ui->cbCapital->isChecked() ? Qt::CaseSensitive : Qt::CaseInsensitive);
        if(index>=0){
...
void ReplaceDialog::on_btReplaceAll_clicked()
{
    ...
    if((pTextEdit !=nullptr)&&(target !="")&&(to !="")){
        ...
        text.replace(target,to,ui->cbCapital->isChecked() ? Qt::CaseSensitive :Qt::CaseInsensitive);
